...
export default function DraftsPage() {
  ...
  return (
    <JourneyShell
      ...
      actions={
        <>
          <div className="flex rounded-full bg-[#FBFBFA] p-1 text-xs">
            {(['bm', 'en'] as const).map(l => (
              <button
                key={l}
                onClick={() => setLanguage(l)}
                className={`px-3 py-1 rounded-full font-bold transition ${
                  language === l ? 'bg-[#173127] text-white' : 'text-[#173127]/50'
                }`}
              >
                {l.toUpperCase()}
              </button>
            ...
            <div key={ch.number} className={`v2-card ${draft?.approved ? 'ring-1 ring-[#C5E86C]' : ''}`}>
              <div className="p-4 flex items-center gap-3 flex-wrap">
                <span
                  className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold shrink-0
                    ${draft?.approved ? 'bg-[#C5E86C] text-[#173127]' : draft ? 'bg-[#173127] text-[#C5E86C]' : 'bg-[#FBFBFA] text-[#173127]/40'}`}
                >
                ...
                >
                  <h3 className="font-semibold text-[#173127] text-sm flex items-center gap-1.5">
                    BAB {ch.number}: {ch.titleBM}
                    {draft && (
                      <ChevronDown
                        className={`w-3.5 h-3.5 text-[#173127]/40 transition-transform ${open ? 'rotate-180' : ''}`}
                      />
                    )}
                  </h3>
              ...
              {busy && (
                <div className="px-4 pb-4">
                  <div className="bg-[#FBFBFA] rounded-xl p-4 flex items-center gap-3">
                    <Loader2 className="w-4 h-4 animate-spin text-[#173127]" />
                    <div className="flex-1">
                      <p className="text-xs font-semibold text-[#173127]">{DRAFT_STAGES[stage]}</p>
                      <p className="text-[10px] text-[#173127]/50">
              ...
              <AnimatePresence>
                {open && draft && !busy && (
                  ...
                  >
                    <div className="border-t border-[#173127]/10 p-5 grid grid-cols-1 lg:grid-cols-3 gap-5">
                      <div className="lg:col-span-2 prose prose-sm max-w-none text-[#173127] prose-headings:text-[#173127] prose-strong:text-[#173127]">
                        <ReactMarkdown remarkPlugins={[remarkGfm]}>{draft.content}</ReactMarkdown>
                      </div>
                      <div className="flex flex-col gap-3 text-xs">
                        <div className="bg-[#FBFBFA] rounded-xl p-3">
                          <p className="font-bold text-[#173127] flex items-center gap-1 mb-2">
                            <Quote className="w-3.5 h-3.5" /> Evidence citations ({draft.citations.length})
                          </p>
                          {draft.citations.length === 0 && (
                            <p className="text-[#173127]/50">No business-specific claims cited.</p>
                          )}
                          {draft.citations.map((c, i) => (
                            <div key={i} className="mb-2 last:mb-0">
                              <p className="text-[#173127]/80 leading-snug">“{c.claim}”</p>
                              <p className="text-[#173127]/50 flex items-center gap-1 mt-0.5">
