...
export async function POST(request: NextRequest) {
  try {
    ...
    if (imageFile) {
      try {
        console.log('Starting image upload, size:', imageFile.size)
        const imageUploadResponse = await jamai.file.uploadFile({
          file: imageFile,
        })
        console.log('Image upload successful, URI:', imageUploadResponse.uri)
        imageUri = imageUploadResponse.uri
      } catch (error) {
        console.error('Image upload failed:', error)
        // Continue without image instead of failing completely
        console.log('Continuing without image attachment')
      }
    }
