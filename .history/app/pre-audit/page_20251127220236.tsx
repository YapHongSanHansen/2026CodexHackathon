...
export default function PreAudit() {
  ...
  // 7 Input file states matching JamAI Base table
  ...
  const [chartFlowFile, setChartFlowFile] = useState<UploadedFile | null>(null)
  const [trainingCertFile, setTrainingCertFile] = useState<UploadedFile | null>(null)
  const [halalPolicyFile, setHalalPolicyFile] = useState<UploadedFile | null>(null)
  const [pestControlFile, setPestControlFile] = useState<UploadedFile | null>(null)
  ...
  // 6 Output columns from JamAI Base
  ...
  const formatChecklistReport = (data: any): string => {
    ...
    try {
      ...
      if (parsed.documents_uploaded !== undefined && parsed.documents_total !== undefined) {
        report += `**Dokumen Dihantar:** ${parsed.documents_uploaded} daripada ${parsed.documents_total}\n\n`
      }
      ...
      // Detailed Findings section - handle checklist array structure
      if (parsed.checklist && Array.isArray(parsed.checklist)) {
        report += `## Senarai Semak Dokumen\n\n`
        parsed.checklist.forEach((item: any, idx: number) => {
          report += `### ${idx + 1}. ${item.requirement_name || item.name || 'Dokumen'}\n\n`
          
          // Status indicator
          const statusEmoji = item.found ? '✅' : (item.skipped ? '⏭️' : '❌')
          const statusText = item.found ? 'Dijumpai' : (item.skipped ? 'Dilangkau' : 'Tidak Dijumpai')
          report += `${statusEmoji} **Status:** ${statusText}\n\n`
          
          // Filename if available
          if (item.filename) {
            report += `**Nama Fail:** ${item.filename}\n\n`
          }
          
          // Note/remarks
          if (item.note) {
            report += `**Catatan:** ${item.note}\n\n`
          }
        })
  ...
  const handleFileUpload = (file: File, specificType?: DocumentType) => {
    ...
    reader.onload = () => {
      ...
      // Handle specific file types matching JamAI Base columns
      if (specificType === 'menu_list') {
        setMenuFile(newFile)
      } else if (specificType === 'ingredient_list') {
        setIngredientFile(newFile)
      } else if (specificType === 'flow_chart') {
        setChartFlowFile(newFile)
      } else if (specificType === 'training_cert') {
        setTrainingCertFile(newFile)
      } else if (specificType === 'halal_policy') {
        setHalalPolicyFile(newFile)
      } else if (specificType === 'pest_control') {
        setPestControlFile(newFile)
  ...
  const removeSpecificFile = (type: DocumentType) => {
    if (type === 'menu_list') {
      ...
      setChartFlowFile(null)
    } else if (type === 'training_cert') {
      setTrainingCertFile(null)
    } else if (type === 'halal_policy') {
      setHalalPolicyFile(null)
    } else if (type === 'pest_control') {
      setPestControlFile(null)
    } else if (type === 'photos') {
      setKitchenPhotos([])
    } else {
  ...
  const getFileForType = (type: DocumentType): UploadedFile | UploadedFile[] | null => {
    ...
    if (type === 'flow_chart') return chartFlowFile
    if (type === 'training_cert') return trainingCertFile
    if (type === 'halal_policy') return halalPolicyFile
    if (type === 'pest_control') return pestControlFile
          ...
          <div className="lg:col-span-6">
            ...
            <div className="flex gap-3">
              ...
              {auditResult.overall_score >= 85 && (
                <button
                  onClick={() => window.open('https://myehalal.halal.gov.my/', '_blank', 'noopener,noreferrer')}
