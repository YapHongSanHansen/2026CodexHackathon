...
export function StatusBadge({ severity, language }: StatusBadgeProps) {
  ...
  const colors = {
    ...
    low: 'bg-[#C5E86C]/30 text-[#173127] border-[#C5E86C]',
  }
    ...
    <div className="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#FBFBFA] flex items-center justify-center">
            <span className="text-xl">{categoryIcons[issue.category]}</span>
          </div>
          <h4 className="font-semibold text-[#173127]">{categoryLabels[issue.category]}</h4>
        </div>
      ...
      <div className="bg-[#C5E86C]/20 rounded-xl p-4 mb-3">
        <p className="text-xs font-semibold text-[#173127] mb-1">
          {language === 'bm' ? 'Cara Betulkan:' : 'How to Fix:'}
        </p>
        <p className="text-sm text-[#173127]">{issue.howToFix}</p>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="font-bold text-[#173127]">{issue.estimatedCost}</span>
      </div>
    </div>
  )
}